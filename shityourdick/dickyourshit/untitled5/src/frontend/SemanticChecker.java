...
//  }
  ...
  public void visit(FuncDefNode node){
    node.returnType.accept(this);
    Scope tmpscope=new Scope(currentScope,node.returnType.type);
    ...
    node.stmts.forEach(stmt -> stmt.accept(this));
    if(currentScope.isReturned==false) {
      if ((!VoidType.equals(node.returnType.type) && !node.name.equals("main"))) {
        throw new BaseError(node.pos, "Function " + node.name + " should have return statement");
      }
    }
    currentScope = currentScope.parentScope;
...
//    currentScope=tmpscope;
    ...
    if (node.classBuild != null) {
      if (node.name.equals(node.classBuild.name)) {
        ...
        throw new BaseError(node.classBuild.pos, "class name does not match");
      }
    }
  ...
  public void visit(BinaryExprNode node) {
    ...
    if (node.lhs.type == null || node.rhs.type == null)
      throw new BaseError(node.pos, "invalid1 expression");
    // 两个 null 不一样
    if (NullType.equals(node.lhs.type) || NullType.equals(node.rhs.type)) {
      ...
      if ((node.op.equals("==") || node.op.equals("!="))
        ...
        throw new BaseError(node.pos, "invalid2 expression");
      }
    }
    if (VoidType.equals(node.lhs.type) || VoidType.equals(node.rhs.type))
      throw new BaseError(node.pos, "invalid3 expression");
